...
function displaySelectedMarker () {
   ...
   const markerFound = markers.find(function(el){return el.key==='11'});
      <Marker title='Meeeee' coordinate={markerFound} ref={markerFound => { markerFound.showCallout}}>
      <Callout>
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style ={{position: 'absolute', top: 65, left: 170, right: 0, bottom: 0, justifyContent: 'top', alignItems: 'center', fontWeight: 'bold', color: 'white'}}>
        ...
        value={name}
        onChange={item => {
          setValue(item.value); displaySelectedMarker(); 
        }}
        renderLeftIcon={() => (
      ...
      <Text style ={{position: 'absolute', top: 590, left: 20, right: 0, bottom: 0, justifyContent: 'center', alignItems: 'center', fontWeight: 'bold', color: 'white'}}>
        BAR STATISTICS: {markers.find(function(el){return el.key==='11'}).website}
        </Text>
    </View>
  );
};
